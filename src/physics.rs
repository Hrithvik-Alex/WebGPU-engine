...
impl PhysicsSystem {
    ...
    const JUMP_VELOCITY: f32 = 25.;
    const JUMP_ACCELERATION: f32 = 100.;
    ...
    fn get_collision_delta(a: &ColliderBoxComponent, b: &ColliderBoxComponent) -> Vector2<f32> {
        ...
        if horizontal_depth < vertical_depth {
            if a.top_right.x > b.bottom_left.x {
                return Vector2::new(horizontal_depth, 0.0);
            } else {
                return Vector2::new(-1.0 * horizontal_depth, 0.0);
            }
        } else {
            if a.top_right.y > b.bottom_left.y {
                return Vector2::new(0.0, -1.0 * vertical_depth);
            } else {
                return Vector2::new(0.0, vertical_depth);
            }
        }
                ...
                if metadata_component.is_controllable() {
                    if input_handler.up_pressed {
                        if !metadata_component.can_jump() {
                            ...
                            physics_component.acceleration.y -= Self::JUMP_ACCELERATION;
                        }
                    }
